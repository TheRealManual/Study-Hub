...
export default function UserProgressUpdateForm(props) {
  const {
    id: idProp,
    userProgress: userProgressModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = userProgressRecord
      ? { ...initialValues, ...userProgressRecord }
      : initialValues;
    setUserID(cleanValues.userID);
    setTopicID(cleanValues.topicID);
    setFlashcardStats(
      typeof cleanValues.flashcardStats === "string" ||
        cleanValues.flashcardStats === null
        ? cleanValues.flashcardStats
        : JSON.stringify(cleanValues.flashcardStats)
    );
    setQuizStats(
      typeof cleanValues.quizStats === "string" ||
        cleanValues.quizStats === null
        ? cleanValues.quizStats
        : JSON.stringify(cleanValues.quizStats)
    );
    setGameStats(
      typeof cleanValues.gameStats === "string" ||
        cleanValues.gameStats === null
        ? cleanValues.gameStats
        : JSON.stringify(cleanValues.gameStats)
    );
    setOverallProgress(cleanValues.overallProgress);
    setLastActivity(cleanValues.lastActivity);
    setCreatedAt(cleanValues.createdAt);
    setUpdatedAt(cleanValues.updatedAt);
    setErrors({});
  };
  const [userProgressRecord, setUserProgressRecord] = React.useState(
    userProgressModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(UserProgress, idProp)
        : userProgressModelProp;
      setUserProgressRecord(record);
    };
    queryData();
  }, [idProp, userProgressModelProp]);
  React.useEffect(resetStateValues, [userProgressRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            UserProgress.copyOf(userProgressRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "UserProgressUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userProgressModelProp)}
